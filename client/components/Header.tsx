...
const Header: React.FC<HeaderProps> = ({ user, onLogout }) => {
  ...
  const roleDotClass = user.role === 'ADMIN' ? 'bg-amber-500' : user.role === 'PERITO' ? 'bg-emerald-400' : 'bg-zinc-500';
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-4">
              <button
                ...
                className="hidden sm:flex items-center gap-2 text-zinc-400 hover:text-amber-400 transition-colors group px-2 py-1 rounded-md hover:bg-white/5"
                title="Trocar senha"
              >
